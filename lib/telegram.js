...
export async function downloadMessageMedia(client, entity, messageId) {
  const messages = await client.getMessages(entity, { ids: [messageId] });
  const msg = messages[0];
  if (!msg?.media) return null;
  const kind = mediaKindOf(msg.media);
  const docMime = docMimeOf(msg.media);
  // GIFs, voice notes and non-animated stickers need the real file; a thumb
  // is useless for audio. Animated .tgs can't render in a browser → thumb.
  const wantFull =
    kind === "gif" ||
    kind === "voice" ||
 ...
 * <a href="…">clickable words</a> arrive as real links.
 */
export async function sendText(client, entity, text, { replyToId, html } = {}) {
 ...
 * waveform), not a file attachment.
 */
export async function sendVoiceNote(client, entity, oggBuffer, { replyToId } = {}) {
