...
function isConnectionError(e) {
  return /AUTH_KEY_DUPLICATED|AUTH_KEY_UNREGISTERED|CONNECTION|disconnect|not connected|timeout|TIMEOUT|socket/i.test(
    String(e?.message || e)
  );
}
...
export async function downloadProfilePhotoSmall(client, cacheKey, chatId) {
  ...
  if (dialog) {
    try {
      const res = await client.downloadProfilePhoto(dialog.entity, {
        isBig: false,
      });
