...
// ---------------------------------------------------------------------------
// Connection pool: connecting to Telegram takes 1-3 seconds, which made every
// API call (and every 1-second poll) slow. Clients are kept connected and
// reused across requests in the same process; one shared connection also
// multiplexes concurrent requests instead of opening parallel connections
// with the same auth key. Stored on globalThis so dev hot-reloads and warm
// serverless instances keep the pool.
// ---------------------------------------------------------------------------

const POOL_IDLE_MS = 10 * 60 * 1000;

function getPool() {
  if (!globalThis.__tgClientPool) globalThis.__tgClientPool = new Map();
  return globalThis.__tgClientPool;
}

function cleanupPool(exceptKey) {
  const pool = getPool();
  const now = Date.now();
  for (const [key, entry] of pool) {
    if (key === exceptKey) continue;
    if (entry.client && now - entry.lastUsed > POOL_IDLE_MS) {
      pool.delete(key);
...
export async function destroyPooled(sessionString) {
  const pool = getPool();
...
async function getPooledClient(sessionString) {
  ...
  if (!entry) {
    entry = { client: null, connecting: null, lastUsed: 0 };
    pool.set(sessionString, entry);
  }
  if (!entry.client) {
    // Concurrent callers share one connect instead of racing.
    if (!entry.connecting) {
      entry.connecting = createClient(sessionString)
        .then((client) => {
          entry.client = client;
...
export async function withClient(sessionString, fn) {
  try {
    ...
    // Connection-level failures: drop the pooled client and retry once with
    // a fresh connection. Real API errors (FLOOD_WAIT etc.) pass through.
    const msg = String(e?.message || e);
    if (
      /AUTH_KEY_DUPLICATED|Not connected|disconnected|Connection closed|TIMEOUT/i.test(
        msg
      )
    ) {
      await destroyPooled(sessionString);
      const client = await getPooledClient(sessionString);
      return await fn(client);
    }
    throw e;
